...
export const AssessmentHistoryTable: React.FC<AssessmentHistoryTableProps> = ({
  ...
  const filteredRecords = records.filter(r =>
    ...
    r.grade_subgrade.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
        <div className="relative w-full sm:w-72">
          ...
          <input
            type="text"
            placeholder="Search by purpose, grade or ID..."
            value={searchTerm}
              ...
              filteredRecords.map((r) => {
                ...
                return (
                  <tr key={r.id} className="hover:bg-slate-900/50 transition">
                    ...
                    <td className="p-3.5 font-medium text-slate-100">${r.annual_income.toLocaleString()}</td>
                    <td className="p-3.5">${r.loan_amount.toLocaleString()}</td>
                    <td className="p-3.5 text-slate-300">{r.loan_purpose}</td>
                    <td className="p-3.5 font-mono text-cyan-400 font-semibold">{r.grade_subgrade}</td>
                    <td className="p-3.5 font-mono">{(r.debt_to_income_ratio * 100).toFixed(1)}%</td>
